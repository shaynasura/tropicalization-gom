...
fish_calculate_gCOB <- function(data = bio_data_clean_all,
  ...
  # Loop to calculate gCOB latitude and longitude for each year when fish appeared in trawl(s)
  for(i in 1:length(unique_years)) {
    ...
    if(nrow(filtered_data) > 0) {
      ...
      # Add extracted coordinates as separate columns
      filtered_data$DECSLON <- coords[, "X"]  # Longitude
      filtered_data$DECSLAT <- coords[, "Y"]  # Latitude
      
      # Calculate weighted geographic centers
...
# ## PREVIOUS VERSION BEFORE CALCULATING gCOBs USING THE SPATIAL GEOMETRIES FOR THESE DATA
# 
# # function to calculate gCOBs for fish species
# fish_calculate_gCOB <- function(data = bio_data_clean_all,
#                                 output_name = "gCOBs_all_data",
