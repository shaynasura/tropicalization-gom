...
library(sf) # to convert to spatial data
...
small_starec_data <- STAREC.csv %>% 
  select(STATIONID, CRUISEID, CRUISE_NO, DEPTH_SSTA, MO_DAY_YR, DECSLAT, DECSLON, DECELAT, DECELON, GEARS, TEMP_SSURF, TEMP_BOT, TEMP_SAIR)
...
# 1. filter out invertebrates and remove extra columns I don't need from BGSREC.csv
bio_data <- BGSREC.csv %>%
  filter(CATEGORY == 3) %>%
...
length(which(is.na(bio_data$HAULVALUE) == TRUE))   # 221,882   # 222,994 on April 18, 2024  # 232,276 on August 26, 2024
...
361748+41+232276  # sum is 594,065 which is the length of bio_data on August 26, 2024
...
write_csv(bio_data, file = "data/bio_data_not_clean.csv")

```



# CLEAN the data based on my observations of fish species names misspellings or taxonomic names that haven't been updated to correspond with FishBase yet
...
- TETRAPTURUS PFLEUGERI -> TETRAPTURUS PFLUEGERI
...
## shouldn't need to clean subsets of data because the entire bio_data should be clean now
# clean_data_to_update <- bio_data_yr2010_2022_m678 # FILL IN HERE
...
bio_data_clean <- st_as_sf(bio_data_clean, coords=c("DECSLON", "DECSLAT"), crs=4326, remove = FALSE)

```


## Export bio_data_clean as .csv file to be used in other scripts
```{r}

# export clean bio data to csv file
write_csv(bio_data_clean, file = "data/clean_data/bio_data_clean.csv")
...
# DON'T DO BELOW - SUBSET DATA IN SEPARATE SCRIPTS LATER ON
...
# Examine data in subsets

## Exclude 1982 - low trawl station coverage of western GoM
## Exclude 2020 - low trawl station coverage in general and no sampling in summer (June/July)
## Exclude 2023 - not all data appears to be entered yet - mostly only from Florida so far
### ^still accurate as of August 26, 2024 - not all 2023 data has been entered yet.
- exclude 1982, (2020), 2023

## There is trawl data from a variety of different months each year - June & July are consistently represented, though, along with October and November.
- analyze only the data from 'summer' trawls (June & July) initially.
- where fish species are caught could differ during the course of year (seasonal ontogeny)

## Eastern side of Gulf of Mexico was not represented in trawl data until mid-2000s
- analyze western and eastern sides of GoM separately


```{r refine clean bio_data}
...
# write_csv(bio_data_post2010, file = "data/bio_data_post2010.csv")
...
# determine longitude cutoff value to analyze the Western GoM across all years
## looks like latitude of -87.4 is a good cutoff to reliably keep most of the trawling stations in the western GoM where they captured fishes starting back to 1982
### take this pre2008_station_data and visualize the maps in the 5_map_stations QMD file

```{r determine longitude cut off for western GoM}

years_to_include <- c(1982:2007)
months_to_include <- c(6,7,8)

pre2008_data <- bio_data_clean %>% 
  filter(YR %in% years_to_include) %>% 
  mutate(MONTH = month(dmy(MO_DAY_YR))) %>% 
  filter(MONTH %in% months_to_include)


pre2008_station_data <- pre2008_data %>% 
  group_by(STATIONID) %>% 
  ...
  mutate(GoM_side = if_else(DECSLON < -87.4, "western", "eastern"))

view(pre2008_station_data)
  
max_longs <- pre2008_station_data %>% 
  group_by(YR) %>% 
  summarize(max_Long = max(DECSLON),
            num_trawls_87.4 = sum(DECSLON < -87.4),
            num_trawls_east_87.4 = sum(DECSLON > -87.4),
            prop_trawls_excluded = (sum(DECSLON > -87.4))/sum(DECSLON != "NA"),
            perc_trawls_excluded = ((sum(DECSLON > -87.4))/sum(DECSLON != "NA"))*100)
