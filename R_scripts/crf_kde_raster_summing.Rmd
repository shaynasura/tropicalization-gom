...
points_sf <- bio_data_final %>% distinct(DECSLON, DECSLAT)

points_sf <- st_as_sf(points_sf, coords = c("DECSLON", "DECSLAT"), crs = 4269)
...
#quick plot
my_extent <- ext(-100, -80, 20, 35)
land_vect_cropped <- crop(land_shapefile, my_extent)


plot(land_vect_cropped, col = "lightgreen", border = "darkgreen", main = "Points and Land Vector")

plot(st_geometry(points_sf), add = TRUE, pch = 19, col = rgb(1, 0, 0, 0.01))
...
land_raster_flipped <- 1 - land_raster
...
plot(land_raster, add = TRUE, col = "white", legend = FALSE)



plot(bio_data_final$YR, bio_data_final$TEMP_SSURF, 
     main = "Scatter Plot of Random Data", 
     xlab = "X-axis (Random Values)", 
     ylab = "Y-axis (Random Values)", 
     pch = 19, 
     col = rgb(0, 0, 1, 0.5))  # Blue points with 50% transparency
abline(bio_data_final$YR~bio_data_final$TEMP_SSURF, col = "red", lwd = 2)  # Red line with thickness 2
