...
min_lat<-min(bio_data_final$DECSLAT)
max_lat<-max(bio_data_final$DECSLAT)
min_long<-min(bio_data_final$DECSLON)
max_long<-max(bio_data_final$DECSLON)

#for testing/debugging
#yr=2015
#sp="PTEROIS VOLITANS"
#bio_data_final<-bio_data_final[1:50000, ] works
#bio_data_final<-bio_data_final[50000:100000, ]  doesn't work
#bio_data_final<-bio_data_final[100000:150000, ]   works
#bio_data_final<-bio_data_final[100000:500000, ] works
...
for (sp in unique(bio_data_final$new_TAXON)) {
  ...
  for (yr in unique(species_data$YR)) {
    ...
    year_data_sf <- st_as_sf(year_data, coords = c("DECSLAT", "DECSLON"), crs = 4269)
    
    # Define the raster extent (you may need to adjust the extent based on your data)
    raster_extent <- ext(min_lat, max_lat, min_long, max_long)

    # Create a raster template with the desired resolution (adjust resolution as needed)
    raster_template <- rast(ext = raster_extent, res = 1)
...
species_raster_stacks$`RHIZOPRIONODON TERRAENOVAE`
